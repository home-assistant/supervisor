...
from supervisor.mounts.mount import BindMount, DiskMount, Mount
...
async def test_load_disk_mount(
    ...
    systemd_service.response_get_unit = {
        "mnt-data-supervisor-mounts-disk_test.mount": [
            ERROR_NO_UNIT,
            "/org/freedesktop/systemd1/unit/tmp_2dyellow_2emount",
        ],
    ...
    assert systemd_service.StartTransientUnit.calls == [
        (
            ...
            [
                ("Type", Variant("s", "ext4")),
                ("Description", Variant("s", "Supervisor disk mount: disk_test")),
        ...
        (
            ...
            [
                ("Options", Variant("s", "bind")),
                ("Description", Variant("s", "Supervisor bind mount: bind_disk_test")),
                ("What", Variant("s", "/mnt/data/supervisor/mounts/disk_test")),
                ("TimeoutUSec", Variant("t", 35000000)),
            ],
            [],
        ),
...
async def test_disk_mount_failed_during_load(
    ...
    unreachable server, so it picks up the reload and remove fixups and the
    periodic retry for free.
    """
    await mock_dbus_services(
        {"systemd_unit": "/org/freedesktop/systemd1/unit/tmp_test"}, dbus_session_bus
    )
    ...
    assert disk_test.state == UnitActiveState.FAILED
    assert (
