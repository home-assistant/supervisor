...
class DockerInterface(CoreSysAttributes):
    ...
    def _start(self) -> None:
        ...
        try:
            ...
            raise DockerError(f"{self.name} not found for starting up", _LOGGER.error) from err
    ...
    def _remove(self) -> None:
        ...
        try:
            ...
            raise DockerError(f"Can't remove image {self.image}: {err}", _LOGGER.warning) from err
    ...
    def _cleanup(self, old_image: Optional[str] = None) -> None:
        ...
        try:
            ...
            raise DockerError(f"Can't find {self.image} for cleanup", _LOGGER.warning) from err

        # Cleanup Current
        try:
            images_list = self.sys_docker.images.list(name=self.image)
        except (docker.errors.DockerException, requests.RequestException) as err:
            raise DockerError(f"Corrupt docker overlayfs found: {err}", _LOGGER.warning) from err
    ...
    def _restart(self) -> None:
        ...
        try:
            ...
            raise DockerError(f"Can't restart {self.image}: {err}", _LOGGER.warning) from err
    ...
    def _stats(self) -> DockerStats:
        ...
        try:
            ...
            raise DockerError(f"Can't read stats from {self.name}: {err}", _LOGGER.error) from err
    ...
    def _get_latest_version(self) -> AwesomeVersion:
        ...
        try:
            ...
            raise DockerNotFound(f"No version found for {self.image}", _LOGGER.info) from err
        except requests.RequestException as err:
            raise DockerRequestError(f"Communication issues with dockerd on Host: {err}", _LOGGER.warning) from err
        else:
