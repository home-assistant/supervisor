...
class APIBackups(CoreSysAttributes):
    """Handle RESTful API for backup functions."""

    def _extract_backup(self, request):
        """Return backup, throw an exception if it doesn't exist."""
        backup = self.sys_snapshots.get(request.match_info.get("backup"))
        if not backup:
    ...
    async def info(self, request):
        """Return backup info."""
        backup = self._extract_backup(request)
