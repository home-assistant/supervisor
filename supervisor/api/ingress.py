...
class APIIngress(CoreSysAttributes):
    ...
    def _extract_addon(self, request: web.Request) -> Addon:
        ...
        if not addon:
            raise HTTPServiceUnavailable(f"Ingress for {token} not available",_LOGGER.warning)
    ...
    async def validate_session(self, request: web.Request) -> dict[str, Any]:
        ...
        if not self.sys_ingress.validate_session(data[ATTR_SESSION]):
            raise HTTPUnauthorized("No valid ingress session {data[ATTR_SESSION]}",_LOGGER.warning)
    ...
    async def handler(
        ...
        if not self.sys_ingress.validate_session(session):
            raise HTTPUnauthorized("No valid ingress session {session}",_LOGGER.warning)
