...
async def _check_connection(websession: aiohttp.ClientSession) -> bool:
    ...
    try:
        async with websession.get(
            _API_CALL.format(hash="00000"), timeout=aiohttp.ClientTimeout(total=5)
        ) as response:
...
async def check_pwned_password(
    websession: aiohttp.ClientSession, sha1_pw: str, offline_mode: bool = True
) -> None:
    ...
    if not await _check_connection(websession):
        ...
        if offline_mode:
            ...
            raise PwnedConnectivityError(
                "No internet connection and offline mode disabled", _LOGGER.warning
            )
